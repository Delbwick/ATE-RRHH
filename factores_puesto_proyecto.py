        ...
        if not factores_df.empty:
            ...
            with col1:
                ...
                if 'complementos_especificos' in factores_df.columns:
                    ...
                    df_especificos['Datos'] = df_especificos['Factor'].apply(lambda x: obtener_datos_tabla(x) if x != 'No disponible' else pd.DataFrame())
                    
                    st.dataframe(df_especificos)
                    
                    for idx, row in df_especificos.iterrows():
                        if row['Factor'] != 'No disponible':
                            df_detalle = obtener_datos_tabla(row['Factor'])
                            st.write(f"Detalles para {row['Factor'].split('.')[-1]}:")
                            st.dataframe(df_detalle)
                
            with col2:
                st.subheader("Selecciona la Letra")
                if 'complementos_especificos' in factores_df.columns:
                    opciones_especificos = df_especificos['Factor'].tolist()
                    for idx, factor in enumerate(opciones_especificos):
                        if factor != 'No disponible':
                            seleccion_letra = st.selectbox(f"Selecciona una letra para {factor.split('.')[-1]}", ['A', 'B', 'C', 'D', 'E'], key=f"especifico_{idx}")
                            df_especificos.at[idx, 'Letra'] = seleccion_letra
            
            # Similar for 'complementos_destino' if needed
